...
# API_KEY = 
INPUT_FOLDER = "papers"                      # Folder containing your PDF files
...
# =====================
EXTRACTION_PROMPT = """
You are an expert biomedical text-mining system.  
Your task is to carefully read a given scientific article and extract all *environmental factors* explicitly mentioned in the text.

## Definition (for guidance only; do not hallucinate)
**Environmental Factors** are external physical, chemical, behavioral, lifestyle, or contextual exposures that may influence health or biological outcomes. These may include:
- Pollutants (e.g., PM2.5, lead, pesticides)  
- Chemicals or toxins  
- Dietary exposures  
- Lifestyle factors (e.g., smoking, alcohol, exercise, sleep)  
- Environmental conditions (e.g., temperature, humidity, radiation)  
- Occupational or geographic exposures  
- Social or built-environment factors if explicitly mentioned  

## Instructions
- Read the provided article carefully.  
- Identify and extract *only* environmental factors explicitly referenced.  
- Do **not** extract genes, proteins, biomarkers, pathways, or other biological entities unless they are described as *environmental exposures*.  
- Avoid interpretation or inference; extract only what is explicitly stated.  
- Output the results strictly in valid JSON with the single key:
